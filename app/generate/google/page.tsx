...
export default function GoogleAdsGeneratePage() {
  const [formData, setFormData] = useState<GoogleAdsGenerationRequest>({
    businessName: '',
    businessDescription: '',
    targetAudience: '',
    keyFeatures: [],
  ...
  const [generatedAds, setGeneratedAds] = useState<GoogleAdsGenerationResponse | null>(null)
  const [isGenerating, setIsGenerating] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [keyFeatureInput, setKeyFeatureInput] = useState('')
  const [keywordInput, setKeywordInput] = useState('')

  const handleInputChange = (field: keyof GoogleAdsGenerationRequest, value: any) => {
  ...
  const addKeyword = () => {
    if (keywordInput.trim() && (formData.keywords?.length || 0) < 10) {
      setFormData(prev => ({
        ...prev,
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await fetch('/api/generate/google', {
        method: 'POST',
        headers: {
  ...
  const exportToCSV = () => {
    ...
    a.href = url
    a.download = `google-ads-${formData.businessName.replace(/\s+/g, '-').toLowerCase()}.csv`
    a.click()
          ...
          <div className="space-y-6">
            <Card className="card-gradient">
              <CardHeader>
                ...
                <p className="text-gray-400">Provide information about your business and campaign goals</p>
              </CardHeader>
                ...
                <div>
                  ...
                  <div className="flex gap-2 mb-2">
                    ...
                    <Button
                      type="button"
                      onClick={addKeyword}
                      disabled={!keywordInput.trim() || (formData.keywords?.length || 0) >= 10}
                      variant="outline"
                      size="sm"
                    >
                      Add
            ...
            {generatedAds ? (
                ...
                <Card className="card-gradient">
                  <CardHeader className="flex flex-row items-center justify-between">
                    <div>
                      ...
                      <p className="text-gray-400 text-sm">Max 30 characters each</p>
                    </div>
                    <Button
                ...
                <Card className="card-gradient">
                  <CardHeader className="flex flex-row items-center justify-between">
                    <div>
                      ...
                      <p className="text-gray-400 text-sm">Max 90 characters each</p>
                    </div>
                    <Button
                  ...
                  <Card className="border-yellow-500/20 bg-yellow-500/5">
                    <CardHeader>
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-lg font-semibold text-yellow-400">Compliance Warnings</h3>
                      </div>
                    </CardHeader>
                    <CardContent>
              ...
              <Card className="card-gradient">
                <CardContent className="text-center py-12">
                  <div className="w-16 h-16 rounded-full bg-gradient-to-br from-blue-500/20 to-cyan-500/20 border border-blue-500/30 flex items-center justify-center mx-auto mb-4">
                    <Chrome className="w-8 h-8 text-blue-400" />
                  </div>
                  <h3 className="text-lg font-semibold text-white mb-2">Ready to Generate</h3>
                  <p className="text-gray-400 text-sm">
                    Fill out the form and click "Generate Google Ads" to create your ad copy
                  </p>
                </CardContent>
              </Card>
            )}
